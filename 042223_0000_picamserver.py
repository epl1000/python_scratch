...
def Printf(format, *args):
    ...
    print("%3d %-4s %7.3f %5.3f " % (line_num, threading.current_thread().getName(), line_time, dt), end='')
    print(format % args)
...
def process_command(conn_com, pack):
    ...
    if pack.type == OmniCamPacketType.CAM_START.value:
        Printf("CAM_START fps=%d jpq_qual=%d", pack.fps, pack.jpg_qual)
        ...
        vid_event.set()
        Printf("    got video streaming true command")
    elif pack.type == OmniCamPacketType.CAM_STOP.value:
        vid_event.clear()
        Printf("    got video streaming false command")
    ...
    while True:
        try:
            ...
            try:
                ...
                Printf("    ERROR receiving command : %s", e)
                break
            ...
            Printf("    Error Receiving Commands: %s", e)
            break
   ...
   # Create a socket object for command receiving
    ...
    while True:
        ...
        try:    
            ...
            Printf("  An unexpected error occurred in accept cmd: %s", e)
...
def send_frames(conn, cap):
    ...
    Printf("  fps = %d jpg_qual = %d", fps, jpg_qual)
    if fps < 1 or fps > 30:
        fps = 15;
    ...
    while True:
        try:
            ...
            # Handle socket error
            Printf("Video Socket ERROR: %s", e)
            break
        except Exception as e:
            # Handle any other unexpected exceptions
            Printf("Send Frame ERROR: %s", e)
            break
